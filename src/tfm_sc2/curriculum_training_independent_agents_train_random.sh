...
# python runner.py --agent_key "${AGENT_KEY}" --map_name "DefeatRoaches" --num_episodes ${TRAIN_EPISODES} --model_id ${MODEL_ID_DEFEAT_ROACHES} --models_path ${MODELS_DIR} --reward_method ${REWARD_METHOD} 2>&1| tee ${LOG_DIR_DEFEAT_ROACHES}/DefeatRoaches${LOG_SUFFIX}.log
# touch ${LOG_DIR_DEFEAT_ROACHES}/_02_training_done_${TRAIN_EPISODES}_ep
